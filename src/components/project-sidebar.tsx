...
import { CheckCircle2, Circle, Rocket, Terminal, Layers, Database, Shield, Zap } from "lucide-react";
...
export function ProjectSidebar() {
  ...
  const projectModules = modules.filter(m => m.lessons.some(l => l.content.includes("## Assignment") || l.content.includes("## Practical Exercise")));
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-sm">
      <Card className="border-2 border-primary/10 shadow-lg bg-primary/5">
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between mb-2">
            <Badge className="bg-primary text-primary-foreground font-mono text-[10px]">Active Project</Badge>
      ...
      <div className="space-y-4">
        <h3 className="font-mono text-xs uppercase tracking-widest text-muted-foreground px-1">Project Roadmap</h3>
            ...
            return (
              ...
              >
                <div className={cn(
                  ...
                  isFullyCompleted ? "bg-green-500 text-white border-green-600" : "bg-background text-primary border-border"
                )}>
                  {isFullyCompleted ? <CheckCircle2 className="h-5 w-5" /> : <Icon className="h-5 w-5" />}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    <p className="truncate text-xs font-bold uppercase tracking-tight">{module.title}</p>
                    {isFullyCompleted && <Badge variant="secondary" className="bg-green-500/10 text-green-600 text-[9px] px-1.5 py-0 h-4">Mastered</Badge>}
                  </div>
                  <div className="mt-1 flex items-center gap-2">
                    <div className="h-1.5 flex-1 rounded-full bg-muted overflow-hidden">
                      <div 
                        className={cn("h-full transition-all duration-500", isFullyCompleted ? "bg-green-500" : "bg-primary")} 
      ...
      <Card className="border-dashed border-2 bg-transparent">
        <CardContent className="p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">Finish 4 more modules to unlock CI/CD.</p>
          </div>
        </CardContent>
      </Card>
    </div>
