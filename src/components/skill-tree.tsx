...
import { Terminal, Layers, Shield, Database, Rocket, Zap, FlaskConical, Container, Trophy, CheckCircle2, Lock } from "lucide-react";
            ...
            return (
              <div key={module.id} className="relative flex flex-col items-center w-full group">
                ...
                <Link
                  ...
                  params={isUnlocked ? { moduleId: module.id, lessonId: module.lessons[0].id } : undefined}
                  className={cn(
                ...
                >
                  ...
                  <div className="mt-2 text-center">
                    <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Module {index + 1}</p>
                  ...
                  <div className="mt-6 flex flex-wrap justify-center gap-2">
                    {isUnlocked ? (
                      <>
                        <Badge variant={isCompleted ? "default" : "outline"} className="text-[10px] py-0 px-2 h-5">
                          {isCompleted ? "Mastered" : `${completedCount}/${lessons.length} Skills`}
                        </Badge>
                        {isStarted && !isCompleted && (
                          <Badge variant="secondary" className="text-[10px] py-0 px-2 h-5">In Progress</Badge>
                        )}
                      </>
                    ) : (
                      <Badge variant="ghost" className="text-[10px] py-0 px-2 h-5 border border-dashed">
